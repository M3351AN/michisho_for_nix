...
--source from strom.lua
require("api_michishio")
local nixc = false
if client.get_username() == "t.me/bat_crack SLAVA UKRAINI!"  then
...
ui.add_slider_int("          [ISONAMI.Kai Ni]", "isonami", 3351, 3351, 3351)
ui.add_slider_int("                      by.m1tzw#", "mitzw", 5953, 5953, 5953)
...
gui = {
    ...
    colors = {
        ...
        elements = col(125, 40, 205),
        --michishio = col(200, 220, 255),
    },
    drag = {
        dragging = false,
...
draggable.new = function (o, size, def_pos)
    local name = "michishio_drag_" .. #draggable.list
...
function gui.render()
    ...
    --local logo_texture = renderer.setup_texture("nix/michishio/logo.png")
...
anti_aim = {
    ...
    last_best_angle = 0,
    types = {"default", "standing", "slow", "run", "air", "duck","air+duck"},
    presets = {},
    elements = {preset = nil},
}
do
    local presets = {
        ...
        {"m1tzw",{att=true,desync={false,37,39,true,48,59,true,58,60,true,49,52,true,32,33,true,31,39,true,22,26,},fakehead={false,false,false,false,},yaw={-4,4,true,-13,13,true,-18,11,true,-4,0,true,0,4,true,-15,18,true,-2,2,},jitter={true,1,0,-5,1,true,1,1,19,3,true,1,1,55,1,true,1,1,55,2,true,1,1,27,1,true,1,1,55,4,true,1,0,53,1,},auto_invert={1,true,3,true,1,true,1,true,1,true,4,true,6,},},},
...
gui.options(gui.checkbox("zuiun roll[Ban?]"), function()
    gui.dropdown("triggers", {"stand", "move", "(fake)duck", "air"}, true)
end):cm(function(cmd, el)
    if anti_aim.in_use then return end
    local lp = entitylist.lp()
    ...
    desync_roll_pitch:set_value(-180)
    if velocity > 50 and triggers.val(1) then
        desync_roll:set_value(32)
    end
    if (velocity < 4 and triggers.val(0)) then
        desync_roll:set_value(73 * inverter)
    end
    if air and triggers.val(3) then
        desync_roll:set_value(29 * inverter)
    end
    if entitylist.lp():movement_type() == 4 and triggers.val(2) then
        desync_roll:set_value(66 * inverter)
    end
...
gui.options("hit visuals", function()
    ...
    -- local font = render.font("nix/michishio/fonts/MuseoSansBlack.ttf", 11, 16)
...
gui.options("world indicators", function()
    local csgo_icons = render.font("nix/michishio/fonts/csgo_icons.ttf", 24, 0)
    ...
    x_mol = gui.options(gui.checkbox("molotov"), function()
        ...
        for i = 1, #infernos do
            pcall(function()
                ...
                renderer.circle(pos, 14, 50, true, col(30, 30, 30, fade * 255))
                local icon = renderer.setup_texture("nix/michishio/inferno.png")
...
indicators.fonts = {
    ...
    pixel = render.font("nix/michishio/fonts/pixel.ttf", 10, render.flags.Monochrome),
}
...
indicators:add(function(ind, pos, a)
    ...
    local fade_colors = {
        col(234, 234, 234),
        custom or gui.colors.michishio,
    }
    ...
    indicators.text(pos + v(6, 0) * (1 - indicators.anim), {
        {"michi", colors.left, alp[1]},
        {"shio", colors.right, alp[2]}
...
indicators:add(function(ind, pos, a)
    ...
    local custom = m_indicators.elems["custom color"]
    local color = (custom:val() and custom:col() or gui.colors.michishio):alpha(a)
    indicators.text(pos, {
        {mode, color, a / 1.5},
...
local m_windows = gui.options(gui.checkbox("windows"), function()
    ...
    info_box.styling = gui.options("styling", function()
        ...
        gui.dropdown("font", {"gamesense", "primordial", "michishio", "onetap"})
    ...
    gui.options("variables", function()    
        gui.dropdown("keybinds name style", {"michishio", "standard", "1x1 2x2 skeet fanboy"}):master(elements, {0})
...
do
    ...
    local watermark_elements = {
        function() return "michishio.lua [alpha]" end,
...
info_box:add(v(90, 90), nil, function(s, pos, size)
    ...
    s.draw[#s.draw+1] = function()
        ...
        local icon = renderer.setup_texture("nix/michishio/running.png")
...
gui.options("esp", function()
    local font = render.font("nix/michishio/fonts/csgo_icons.ttf", 14, 0)
    local icons = {
        ["decoy"] = "F",
        ["flashbang"] = "G",
        ["hegrenade"] = "H",
...
gui.options("additional", function()
    ...
    local m_killmarks = gui.checkbox("killmarks [!]"):paint(function(el)
        ...
        local texture = renderer.setup_texture(string.format("nix/michishio/killmarks/%s.png", km.count))
        if km.count > 0 and km.last + 2.0 > globalvars.get_current_time() and km.head == 1 then
            texture = renderer.setup_texture("nix/michishio/killmarks/hs.png")
...
gui.options(gui.checkbox("ragdoll"), function()
    gui.dropdown("type", {"none", "neverlose", "michishio"}):change(function(on)
...
gui.checkbox("console filter"):change(function(on)
    ...
    se.get_convar("con_filter_text"):set_string(on and "michishioluadabest" or "")
    se.get_convar("con_filter_text_out"):set_string(on and "michishioluadabested" or "")
end)
gui.checkbox("thunder sound"):paint(function()
...
engine.execute_client_cmd("play buttons/arena_switch_press_02")
