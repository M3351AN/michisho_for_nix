...
ffi.C.CreateDirectoryA("nix/michishio/", nil)
ffi.C.CreateDirectoryA("nix/michishio/fonts/", nil)
ffi.C.CreateDirectoryA("nix/michishio/flags/", nil)
...
entity_t.movement_type = function(s)
    ...
    if (s:in_air() or (is_lp and client.is_key_pressed(32) and extended:is_window_active())) 
        and (s:get_prop_float(m_.flDuckAmount) > 0.1 or (is_lp and fakeduck_bind:on())) then return 5 end--air+duck
    if s:in_air() or (is_lp and client.is_key_pressed(32) and extended:is_window_active()) then return 3 end--air
    if s:get_prop_float(m_.flDuckAmount) > 0.1 or
        (is_lp and fakeduck_bind:on()) then return 4 end--duck
    if slowwalk_bind:is_active() and s:velocity() > 5 then return 1 end--walk
    if s:velocity() < 5 then return 0 end--stand
    if s:velocity() >= 5 then return 2 end--run
end
...
indicators = {
    list = {},
    -- fonts = {
    --     -- {"nix/michishio/fonts/MuseoSansBlack.ttf", render.flags.LightHinting},
    --     -- {"C:/windows/fonts/trebucbd.ttf", render.flags.ForceAutoHint},
    --     -- {"C:/windows/fonts/courbd.ttf", render.flags.MonoHinting},
    --     {"nix/michishio/fonts/smallest_pixel-7.ttf", render.flags.Monochrome},
...
extended = {
    ...
    ["log"] = function(text, color)
        ...
        extended.print_color("michishio.lua | ", col(210, 210, 220))
        extended.print_color(text.."\n", color)
    end,
    ["log_multi"] = function(text, c)
        if c == nil then c = col(210, 210, 220) end
        extended.print_color("michishio.lua | ", c)
    ...
    ["rgba_to_texture"] = function(data)
        ...
        extended.exec("\""..nix_path.."/michishio/convert.exe\" \"\""..tmp.."\"\" \"\""..tmp..".png\"\"")
        return tmp..".png"
    end,
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        col(145, 197, 56),
        "welcome to michishio for nixware. ",
        col(255, 255, 255),
        username:lower(),
        col(145, 197, 56),
